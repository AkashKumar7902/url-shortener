...
The independent reviews found incorrect output. Two concurrent requests for one URL that generated the same candidate could make the loser retry until exhaustion. I accepted the correction in both layers: the store prioritizes the generated-URL invariant, and the service re-reads the winner after a code conflict. A barrier-based race test reproduces that interleaving. Another review found that public origins ending in an empty `?` or `#` produced malformed short URLs; those forms are rejected. Post-rename handling was also changed so a directory-sync failure makes that store instance unavailable rather than letting a retry acknowledge uncertain durability. Raw Unicode and empty-port cases were tightened after black-box review.
